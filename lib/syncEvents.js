  ...
  return text.replace(/&(#?[\w\d]+);/g, (match, entity) => {
    if (entity.charAt(0) === '#') {
      const code = entity.charAt(1).toLowerCase() === 'x'
        ? parseInt(entity.slice(2), 16)  // hex
        : parseInt(entity.slice(1), 10); // decimal
...
const entities = {
  ...
  // Accented characters (frequent in UK/European contexts)
  aacute: 'á',
  eacute: 'é',
  iacute: 'í',
  oacute: 'ó',
...
for (const event of events) {
  ...
  const doc = {
    ...
    dateTime: parseDateTime(event.EventStartDate, event.EventStartTime),
